...
void main() {
  runApp(const semesterBScreen());
}

class semesterBScreen extends StatefulWidget {
  const semesterBScreen({Key? key}) : super(key: key);

  @override
  semesterBScreenstate createState() => semesterBScreenstate();
}

class semesterBScreenstate extends State<semesterBScreen> {
  @override
  double gpa = 0;
  double course_num = 0;
  double hour = 0;
  ...
  double gp = 0;
  double totalhour = 0;
  final items = ["A+", 'A', "A-", 'B+', "B", "B-", 'C+', "C", "C-", "D+","D" ,"F"];
  var gpp = {
    "A+": 4,
    "A": 3.667,
    "B+": 3.334,
